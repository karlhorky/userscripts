...
function observe(maxMutations, selectors, callback) {
  ...
  observer = new MutationObserver(() => {
    ...
    if (mutations >= maxMutations) {
      console.log(
        `${logPrefix}disconnecting MutationObserver after ${mutations} mutations to avoid slowing down the page`,
      );
      observer.disconnect();
    }
  ...
  function navigated() {
    console.log(
      `${logPrefix}navigated, resetting alreadyClicked and MutationObserver`,
    );
    observer.disconnect();
    resetAlreadyClicked();
    mutations = 0;
  ...
  if (window.navigation && window.navigation.addEventListener) {
    ...
    console.log(
      `${logPrefix}using setInterval(..., 1000) to detect location changes`,
    );
    ...
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) {
        ...
        console.log(
          `${logPrefix}page has become visible, starting setInterval`,
        );
        startPollingLocation();
      }
    });
...
//   observe(100, ['.inline-show-more-text__button'], (el) => {
//     el.click();
//   });
// }
